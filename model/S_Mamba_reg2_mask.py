...
class EncoderLayer(nn.Module):
    def __init__(self, attention, attention_r, d_model, d_ff=None, dropout=0.1, activation="relu",p=0.2):
    ...
    def random_mask(self,x_,p=0.2):
        #m = x_.mean(dim=2, keepdim=True)
        N = int(x_.shape[1]*p)
        idx_mask = torch.randint(0,x_.shape[1],(N,))
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None,train=True,mi=False):
        ...
        if train:
            ...
            x1 = self.attention(self.random_mask(x,p=self.p)[:, idx1, :])[:, idx_inv1, :]
            x2 = self.attention(self.random_mask(x,p=self.p)[:, idx2, :])[:, idx_inv2, :]
            loss2 = torch.sqrt(torch.sum((x1 - x2) ** 2)/(x.shape[0]*x.shape[1]))
            x = x + (x1+x2)
        else:
            if mi:
