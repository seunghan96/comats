...
class EncoderLayer(nn.Module):
    def __init__(self, attention_local, attention_r_local, 
                 attention_global, attention_r_global,
                 d_model, d_ff=None, dropout=0.1, activation="relu"):
        super(EncoderLayer, self).__init__()
        d_ff = d_ff or 4 * d_model
        self.attention_local = attention_local
        self.attention_r_local = attention_r_local
        self.attention_global = attention_global
        self.attention_r_global = attention_r_global
    ...
    def forward(self, x_local, x_global, attn_mask=None, tau=None, delta=None):
        x_local2 = self.attention_local(x_local) + self.attention_r_local(x_local.flip(dims=[1])).flip(dims=[1])
        x_global2 = self.attention_global(x_global) + self.attention_r_global(x_global.flip(dims=[1])).flip(dims=[1])
        attn = 1
        x = (x_local+x_local2) + (x_global+x_global2)
        y = x = self.norm1(x)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Embedding
        self.enc_embedding_local = DataEmbedding_inverted(configs.seq_len, configs.d_model, configs.embed, configs.freq,
                                                    configs.dropout)
        self.enc_embedding_global = DataEmbedding_inverted(configs.seq_len, configs.d_model, configs.embed, configs.freq,
                                                    configs.dropout)
    ...
    def forecast(self, x_enc_local, x_enc_global, x_mark_enc):
        if self.use_norm:
            ...
            means = (x_enc_local+x_enc_global).mean(1, keepdim=True).detach()
            x_enc_local = x_enc_local - means
            x_enc_global = x_enc_global - means
            stdev = torch.sqrt(torch.var((x_enc_local+x_enc_global), dim=1, keepdim=True, unbiased=False) + 1e-5)
        ...
        enc_local_out = self.enc_embedding_local(x_enc_local, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens
        enc_global_out = self.enc_embedding_global(x_enc_global, x_mark_enc) # covariates (e.g timestamp) can be also embedded as tokens
